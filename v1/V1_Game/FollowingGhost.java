...
public class FollowingGhost extends Ghost{
	...
	public void move(Vector target, ArrayList<Wall> walls) {
		...
		if (counter == 0) {
			...
			for (Wall w : walls) {
				if ((w.x == u.x && w.y == u.y) || u.equals(prevPos)) {
					uOkay = false;
				}

				if ((w.x == d.x && w.y == d.y) || d.equals(prevPos)) {
					dOkay = false;
				}

				if ((w.x == l.x && w.y == l.y) || l.equals(prevPos)) {
					lOkay = false;
				}

				if ((w.x == r.x && w.y == r.y) || r.equals(prevPos)) {
					rOkay = false;
				}
			}
			...
			u.sub(t);
			if(!uOkay) {
				u.setMag(10000000000000.0);
			}
			d.sub(t);
			if(!dOkay) {
				d.setMag(10000000000000.0);
			}
			l.sub(t);
			if(!lOkay) {
				l.setMag(10000000000000.0);
			}
			r.sub(t);
			if(!rOkay) {
				r.setMag(10000000000000.0);
			}
